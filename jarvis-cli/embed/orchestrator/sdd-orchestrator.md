...
3. Cache the skill index rows, including trigger/description and exact `SKILL.md` paths
...
**Key rule**: inject exact `SKILL.md` paths as the primary contract. Sub-agents read those files before task-specific work. Compact rules may remain transitional metadata, but they do not replace path injection.
