...
function resolveHiveSessionId(input: unknown, output: unknown): string {
  ...
  const session = readPath(input as any, output as any, [
    "session_id",
    "sessionId",
    "sessionID",
  ]);
...
function resolveHiveDirectory(input: unknown, output: unknown): string {
  ...
  const directory = readPath(input as any, output as any, [
    "directory",
    "cwd",
    "workspace",
  ]);
  ...
  return {
    "chat.message": async (input: unknown, output: any) => {
      ...
      try {
        ...
        const response = await fetch(HIVE_URL, {
          method: "POST",
        ...
        if (!response.ok) {
          console.warn(
            `Hive prompt capture rejected: endpoint=${HIVE_URL} status=${response.status}`,
          );
        }
      } catch {
