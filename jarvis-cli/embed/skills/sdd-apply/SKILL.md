...
- On `continuation_required`, the prefix is committed: retain the receipt and resume only from the returned snapshot, expected coordinates, `next_entry_index`, `next_entry_id`, and `stream_sha256`; preserve and supply `stream_sha256` unchanged with the complete ordered entry stream. Use a new request ID and new batch ID only after `continuation_required`.
