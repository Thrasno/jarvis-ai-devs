...
- Use only an exact closed-catalog Deluge task or a verified exact REST operation from official evidence. A family or neighboring operation never authorizes an endpoint.
...
| Verified REST operation | Use that exact method, path, version, hierarchy, and scopes from official operation evidence. |
...
3. Select one exact Deluge task or verified REST operation; reject incompatible context, version, plan, or availability.
