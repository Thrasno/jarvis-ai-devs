...
Resolve or adopt the immutable `project/change` binding before selecting an archive backend. A persisted `hive`, `openspec`, or `hybrid` binding wins over `JARVIS_SDD_STORE_MODE`; `none` is never persisted. Hive binding belongs in SQLite, OpenSpec binding belongs in `openspec/changes/{change-name}/state.yaml`, and hybrid requires matching copies. Treat unavailable/protocol failure as distinct from absence. Malformed, unsupported, noncanonical, divergent, or protected-progress state blocks; hybrid never chooses a copy as a winner.
...
A superseded predecessor cannot be archived, even if its old tasks were complete. Use `jarvis sdd supersede --change <predecessor> --successor <successor> --actor <actor> --reason <reason>` for a bound partial predecessor, not archive. New seals require `--actor` and `--reason`; strict predecessor and successor preflight precedes consent. Signed credited tasks require typing the exact successor name; zero credit skips the prompt only after preflight. Retry with the same successor and omitted attribution flags or their exact signed values. The seal preserves the predecessor's original manifest and immutable history; fresh successor progress starts at generation 1/revision 1 without transferred credit. Hybrid publication is not distributed-atomic: replay exact signed intent on retry, never choose a backend winner. Consent is not archive authority; never archive the predecessor.
...
- The archive-report must be complete and executor-written. It is the Hive logical-closure artifact; do not introduce a typed closure API, closure state, or archive receipt.
...
- Hybrid requires byte-identical and non-blank archive reports plus equivalent protected progress on both sides. Revalidate both immediately under the local archive lock; never choose one copy or claim distributed atomicity. Hive writes after the final Hive fetch are outside that local lock.
