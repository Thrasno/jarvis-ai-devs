...
A superseded predecessor cannot be archived, even if its old tasks were complete. The current supersession consent gate is local only; no public successor route exists yet. After all prechecks, nonzero credited tasks require typing the exact successor name; zero credit skips the prompt. Sealing preserves the predecessor's original manifest, and fresh successor progress starts at generation 1/revision 1 without transferred credit. Do not treat consent as archive authority.
