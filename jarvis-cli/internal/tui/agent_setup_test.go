...
func TestConfigureWizardAgents_AggregatesResults(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			results := configureWizardAgents(tt.agents, state.PhaseModels{}, agent.MCPEntry{Name: "hive"}, agent.MCPEntry{Name: "context7"}, tt.resolved, wizardPresetApplyContext{}, testSkillsFS, nil, nil, func() bool { return true }, false, "")
...
func TestConfigureWizardAgentsUsesCanonicalProfile(t *testing.T) {
	...
	results := configureWizardAgents([]agent.Agent{stub}, state.PhaseModels{}, agent.MCPEntry{Name: "hive"}, agent.MCPEntry{Name: "context7"}, resolved, wizardPresetApplyContext{Layer1: "layer1"}, testSkillsFS, nil, nil, func() bool { return true }, false, "")
...
func TestConfigureWizardAgents_RuntimeVerification(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			results := configureWizardAgents([]agent.Agent{tt.agent}, state.PhaseModels{}, agent.MCPEntry{Name: "hive"}, agent.MCPEntry{Name: "context7"}, nil, wizardPresetApplyContext{}, testSkillsFS, nil, nil, func() bool { return true }, false, "")
			if len(results) != 1 {
				t.Fatalf("len(results) = %d, want 1", len(results))
			}
...
func TestConfigureWizardAgents_RuntimeVerificationUsesPendingConfigForOpenCodeDefault(t *testing.T) {
	...
	results := configureWizardAgents([]agent.Agent{a}, pendingModels, agent.MCPEntry{Name: "hive"}, agent.MCPEntry{Name: "context7"}, nil, wizardPresetApplyContext{}, testSkillsFS, nil, nil, func() bool { return true }, false, "")
	if len(results) != 1 {
		t.Fatalf("len(results) = %d, want 1", len(results))
	}
