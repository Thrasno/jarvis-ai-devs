...
func (s OpenSpec) advance(request AdvanceRequest, legacy []byte) (AdvanceResult, error) {
	...
	replayed, err := s.checkReceipt(request.RequestID, payload)
	if err != nil {
		return AdvanceResult{}, err
	}
	...
	if replayed && current != nil && current.Digest == snapshot.Digest {
	...
	if err := writeImmutable(filepath.Join(s.Root, ".apply-progress-receipts", request.RequestID+".json"), []byte(`{"payload":"`+payload+`"}`), s.syncDir, s.BeforeImmutablePublish); err != nil {
		return AdvanceResult{}, err
	}
...
func writeImmutable(path string, data []byte, sync func(string) error, beforePublish func() error) error {
	...
	if err := os.Link(name, path); os.IsExist(err) {
