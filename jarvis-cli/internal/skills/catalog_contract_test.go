...
func TestCatalogContract_SDDApplyCommitsProgressBeforeMarkingTasksComplete(t *testing.T) {
	...
	progressStep := strings.Index(content, "### Step 5: Persist Progress")
	checkboxStep := strings.Index(content, "### Step 6: Mark Tasks Complete")
	if progressStep < 0 || checkboxStep < 0 || progressStep > checkboxStep {
		t.Fatal("expected guarded progress to be committed before task checkboxes are marked complete")
	}
	if strings.Contains(content, "Mark task as complete [x] in the persisted tasks artifact immediately") {
		t.Fatal("expected task checkboxes to remain unchecked until guarded progress succeeds")
	}
}
