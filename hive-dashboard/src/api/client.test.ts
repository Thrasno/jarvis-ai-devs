...
describe('Hive API client', () => {
  ...
  it('serializes the canonical degraded health filter', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ projects: [], total: 0 }))

    await createApiClient({ fetch: fetchMock }).projects('jwt-token', { health: 'degraded' })

    expect(fetchMock).toHaveBeenCalledWith('/projects?health=degraded', {
      method: 'GET',
      headers: { Authorization: 'Bearer jwt-token' }
    })
  })
