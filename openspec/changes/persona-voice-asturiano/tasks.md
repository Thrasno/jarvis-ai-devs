...
## Phase 1: RED — Update Existing Test (falsify relabel)

- [x] 1.1 In `jarvis-cli/internal/persona/v2_test.go`, update `TestBoundDialectClauseUsesReadableLanguageName` (~line 420): change `"asturiano": "Asturian"` → `"asturiano": "Asturian Spanish"`. Confirm `rawEnums` (line 423, includes `"es-asturian"`) still passes since `"Asturian Spanish"` does not contain `"es-asturian"`.
- [x] 1.2 Run `go test ./internal/persona/... -run TestBoundDialectClauseUsesReadableLanguageName` — confirm RED (fails against current `"Asturian"` return in loader.go).
...
## Phase 3: GREEN — Fill Prose Maps and Relabel

- [x] 3.1 In `jarvis-cli/internal/persona/loader.go` (~line 180-181), change the `case "es-asturian":` arm of `presentationLanguage` to `return "Asturian Spanish"`. Leave `es-rioplatense` and `es-galician` arms untouched.
