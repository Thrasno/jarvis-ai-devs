...
| Asturiano Dialect-Gating Clause — names the Asturian layer, gates on Spanish | `presentationLanguage("es-asturian")` stays `"Asturian"`; asserted via `"- Dialect gating: the Asturian dialect layer"`, `"applies only when replying in Spanish"`, and via `TestBoundDialectClauseUsesReadableLanguageName["asturiano"] == "Asturian"` | ✅ PASS |
