...
Add shared protocol code in `hivederive/applyprogress`; both daemon and CLI use identical canonicalization/validation. Executors checkpoint only through `jarvis sdd progress checkpoint`, which plans one bounded prefix and delegates publication to the configured store. `advance` and the HTTP/MCP advance surfaces remain low-level compatibility boundaries for caller-proposed snapshots and batches; they never plan checkpoints. Every lifecycle read resolves one snapshot and exactly its referenced immutable batches before routing.
...
`Snapshot{Schema,Project,Change,Generation,Revision,PreviousDigest,TaskManifestSHA256,Status,Coverage,Batches,StreamSHA256?,NextEntryIndex?,NextEntryID?,Digest}`; `Coverage{TaskID,BatchID,EntryID}`; `BatchRef{BatchID,SHA256}`. `Batch{Schema,Project,Change,BatchID,Entries,SHA256}`; `EvidenceEntry{EntryID,TaskIDs,CompletesTaskIDs,Kind,Summary,Command,ExitCode,Outcome,Files}`. Slices are non-nil and maps are forbidden. The three continuation fields are an all-or-nothing optional group: all are omitted for terminal and pre-continuation v2 snapshots, preserving their canonical bytes; otherwise they bind the full ordered stream and the next cursor. Status is `partial|complete`; kind is `red|green|triangulate|refactor|verification|delivery|imported`; outcome is `pass|fail|not_run`. `imported` records verified legacy provenance and makes no TDD quality claim. IDs match `[A-Za-z0-9][A-Za-z0-9._-]{0,63}`; batches are `apb-` + 32 lowercase random hex; entry IDs are unique across the complete ordered evidence stream, including every referenced batch.
...
Validation order: canonical decode/re-encode; schema/identity/limits; hashes; current manifest digest; unique ordered refs; fetch only referenced batches; verify batch identity/bytes/hash; reconcile entries in ref/entry order. Every attributed task must exist; each completed task appears once in `CompletesTaskIDs`; `Coverage` must equal manifest-ordered completion triples; `complete` requires full coverage, while `partial` may have full coverage only with the atomic continuation cursor proving trailing evidence remains. A partial snapshot without continuation is an exhausted incremental stream awaiting a future stream and therefore has incomplete coverage. Extra/orphan batches are ignored; missing, duplicate, reordered/forked, corrupt, or edited-task states return typed reasons.

## Data Flow and Persistence

```text
executor → CLI checkpoint coordinator → plan one prefix → OpenSpec and/or Hive CAS
                                      ↓                 ↓
status/apply/verify/archive ← resolve + lazy validate ← snapshot+batches
```

SQLite migration in existing `hive-daemon/internal/db/db.go` adds `sdd_apply_heads(project,change_name,snapshot_memory_id,generation,revision,digest,PRIMARY KEY(project,change_name))` and `sdd_apply_receipts(request_id PRIMARY KEY,project,change_name,payload_sha256,response_json,created_at)`. The receipt retains one bounded response payload so a client that loses the commit response can replay without reconstructing an outcome; `request_id` is intentionally globally unique within one daemon, preventing reuse across projects or changes. Receipts have no time-based GC today: OpenSpec receipts move with their archived topology, while Hive retains one small row per global request ID for the database lifetime. New `internal/db/apply_progress.go` performs one transaction: receipt lookup; expected generation/revision/digest check; byte-identical same-ID batch reuse; memory rows/journal events; head update; receipt. Generation identifies the lineage epoch and remains unchanged for ordinary successors; revision is the CAS coordinate and advances from expected revision to expected revision+1. Retrying identical request returns the stored response; changed payload returns `request_id_conflict`; stale state commits nothing. Pulled v2 rows rebuild only a unique validated digest chain; forks stay invalid. Progress documents remain normal immutable memory rows and therefore participate in existing FTS indexing: this accepts modest index/storage amplification for diagnostics while guarded heads and referenced batches, never FTS results, remain the sole authority. Project migration rebuilds those FTS indexes after rekey because external-content rows and their triggers are keyed by memory IDs; rebuilding preserves search consistency after table replacement rather than relying on stale index contents.
...
| `hive-daemon/internal/db/{db.go,sdd.go,apply_progress.go}`; `internal/{governance/sdd.go,mcp/server.go,mcp/tools.go,httpapi/server.go}` + tests | existing modify: guarded head plus evidence and receipt-identity readers |
| `hive-daemon/internal/db/apply_progress.go` + tests | proposed new: cohesive CAS/retrieval |
| `jarvis-cli/internal/hiveclient/client.go`, `internal/sddstatus/{source,status}.go`, `cmd/jarvis/cmd_sdd.go` + tests | existing modify: bounded evidence and receipt-identity reader clients feed typed status |
...
The checkpoint writer has exactly six domain outcomes:
...
| `stream_preflight_required` | No write. A newly bound stream's simulated maximal-batch continuation cannot reach a bounded terminal snapshot because future references or coverage overflow it. | Current state, exact capacity detail, and recovery to consolidate evidence or evolve the snapshot before binding the stream. |
| `checkpoint_consolidation_required` | No write. An unbound near-limit base would consume another reference with an undersized nonterminal batch. | Reference and batch-size details directing the caller to consolidate before binding a stream. |
...
3. After each addition, seals the candidate batch with existing canonical JSON/SHA-256 code, appends its reference, updates manifest-ordered coverage, retains the generation epoch, increments revision by one, and seals the candidate snapshot.
...
6. Before a new stream's first continuation binding, simulates the full maximal-batch continuation. If future snapshot references or coverage overflow capacity, returns `stream_preflight_required` without a planned batch or snapshot; active continuations and exhausted incremental streams retain normal incremental behavior.
7. Otherwise returns one batch and one next snapshot. The coordinator emits `committed` only after `Advance` confirms the whole remaining stream was committed, or `continuation_required` only after `Advance` confirms the planned prefix was durably committed.
...
- **Planner capacity or stream-preflight result:** call no backend and create no receipt, batch, snapshot, or completion coverage. `stream_preflight_required` is evaluated only before a new continuation binding; a prefix already committed by an earlier call remains authoritative.
...
- **Legacy import and pre-continuation v2 snapshot:** an unambiguous legacy Markdown artifact is automatically imported on the first checkpoint with `imported` evidence, while its original bytes remain authoritative until the guarded commit succeeds. An all-done legacy source with supplied new entries, cursor, or stream identity is rejected before write with typed legacy-migration output; no supplied payload is silently dropped. A readable partial v2 snapshot without stream/cursor fields is different: it remains readable but cannot resume until `jarvis sdd progress upgrade-continuation --root <change-root> --request <request.json>` executes its pre-continuation CAS. The canonical request supplies the exact current base, tasks, complete ordered entry stream, stream SHA-256, and fresh request ID. The mutation preserves existing batches and coverage exactly, keeps generation stable, increments revision, and changes only the atomic continuation identity and normal CAS envelope; immutable evidence does not block this recovery.
...
| --- | --- |
| `hivederive/applyprogress/checkpoint.go`, `checkpoint_test.go`, and `tasks.go` | Pure checkpoint boundary planner, the exact six checkpoint outcomes, cursor validation, one-batch maximal-prefix fitting, task parsing, and exact-rune tests. Existing model/canonical/validation behavior is reused, not copied. |
...
### Shipping Implementation Clarification

The shipped checkpoint planner is flat: it seals complete ordered entries into canonical batches and has no tree or inclusion-proof mechanism. It returns exactly six domain dispositions: `committed`, `continuation_required`, `evidence_item_too_large`, `snapshot_capacity_exhausted`, `stream_preflight_required`, and `checkpoint_consolidation_required`.

Capacity is refused before publication. For `snapshot_capacity_exhausted`, the response calculates `capacity.current_runes` from the persisted authoritative head, not an in-memory proposal. Preflight and consolidation also refuse before a batch, snapshot, receipt, completion, or coverage write. If a change cannot complete because of capacity, it closes with apply `StatusPartial`, is not archived, and carries task IDs absent from coverage into a new ordinary SDD change.

The low-level `advance` writer remains unchanged. HTTP and MCP keep progress, evidence, and receipt GET surfaces alongside low-level advance compatibility; none of those boundaries plans prefixes. Apply, verify, and archive consume shared bounded-progress guidance. Deterministic JSON contract fixtures bind the daemon HTTP retrieval responses and the CLI consumers to the same wire bytes.
...
| --- | --- | --- | ---: |
| C1 — pure planner contract | Add planner types/algorithm in `hivederive/applyprogress`; prove whole-entry maximal prefixes, cursor checks, the exact six checkpoint outcomes, new-stream preflight, Unicode limits, and pure repeated planning over 162,725 runes. | `cd hivederive && go test ./applyprogress -count=1 && go vet ./...`; remove only planner files. | 300–390 |
