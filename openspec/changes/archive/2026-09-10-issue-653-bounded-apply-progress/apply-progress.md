...
- Added `hivederive/applyprogress/checkpoint.go`: at the historical PR 6A boundary, pure one-batch maximal-prefix planning sealed existing canonical batch/snapshot values, preserved manifest-ordered coverage, and returned the then-implemented subset `committed`, `continuation_required`, `evidence_item_too_large`, or `snapshot_capacity_exhausted`. The current canonical contract extends that subset with `stream_preflight_required` and `checkpoint_consolidation_required`.
