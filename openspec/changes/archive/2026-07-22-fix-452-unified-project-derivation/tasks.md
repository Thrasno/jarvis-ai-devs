...
- [x] 4.5 RED/regression: `doc_claim_test.go` (root `jarvis_test` package in `jarvis-cli`) fails if any false registration claim reappears in either embed doc, and positively asserts the SessionStart-hook mechanism is documented.
