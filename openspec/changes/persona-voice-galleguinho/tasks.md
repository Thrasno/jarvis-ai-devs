...
- [x] 1.1 In `jarvis-cli/internal/persona/v2_test.go` `TestBoundDialectClauseUsesReadableLanguageName` (~line 421), no change needed — `"galleguinho": "Galician"` stays as-is (an earlier relabel attempt was reverted; the readable label remains "Galician").
- [x] 1.2 Run `go test ./jarvis-cli/internal/persona/... -run TestBoundDialectClauseUsesReadableLanguageName` and confirm it still PASSES (loader.go label unchanged).
...
- [x] 3.1 In `jarvis-cli/internal/persona/loader.go` `presentationLanguage` `case "es-galician"` (~line 182), no change — return value stays `"Galician"` (no relabel). Do NOT touch `es-rioplatense`/`es-asturian` arms.
