...
- Known residual risk: the committed resolver directory lock is not context-cancelable, so command-level timeouts do not bound lock contention. No unsafe goroutine timeout was introduced; cancellation/timeout semantics require a separate reviewed lock work unit.

## Next step

Address the resolver lock-cancellation risk as a separate bounded work unit, then begin ODD-723-04 protected-write and archive routing through the persisted binding. Keep #722 identity/purge ownership and #724 supersession guards out of both slices.
