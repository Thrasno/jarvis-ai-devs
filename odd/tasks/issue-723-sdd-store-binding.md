...
- [x] **ODD-723-05 — Update diagnostics, guidance, and regression coverage**
  - Route: delegated direct writer; multi-file write trigger.
...
- [x] **ODD-723-06 — Final verification and delivery preparation**
...
- ODD-723-05 cross-command regression and recovery correction are committed as `0fc72bc9` (`fix(sdd): recover bound hybrid progress after restart`). Fresh command/client instances prove persisted Hive, OpenSpec, hybrid, and unpersisted none selections survive environment changes without an accidental second-store write. The hybrid case exposed an ordinary v2 partial receipt being misclassified as legacy recovery; the correction classifies an exact payload before consulting legacy authority, preserves the receipt schema, and retains fail-closed legacy derivation. The final HTTP mock returns exact state/receipt identities and the negative case proves a mismatched Hive acknowledgement remains failed. Focused x20, package, race, LSP, independent verification, and native review lineage `review-1a98137726ae243f` passed and were acknowledged.
- ODD-723-05 source guidance is committed as `39033ffe` (`docs(sdd): explain persistent store bindings`). Embedded persistence, phase, orchestrator, apply, and archive sources plus the user guide now describe immutable backend-owned binding, persisted-over-environment precedence, status adoption, legacy protected-progress authority, fail-closed hybrid equality, existing progress receipt recovery, executor-authored archive reports, and the local-lock/distributed-atomicity limit. Asset regression guards reject Hive-first fallback, purely read-only status, and missing-side-only replay wording. Independent verification found and corrected replay-versus-rewrite guidance; final verification and native review lineage `review-c231a7305d070eb2` passed and were acknowledged.
- ODD-723-06 rebased the 23 #723-owned commits from the historical #722 boundary onto merged `public/master` `a3aad9f1`, removing #722 identity/promotion commits from the delivery diff while retaining the Hive DB foundation already merged through #722. The stale preflight contracts exposed by the full CLI suite were corrected in `4cf374cd` (`test(sdd): align status adoption contracts`); focused x20, full CLI, vet, independent verification, and native review lineage `review-ab0171754e81d070` passed and were acknowledged.
- Final independent verification passed `go test -count=1 ./...` and `go vet ./...` in `hivederive`, `hive-daemon`, `jarvis-cli`, and `hive-api`; focused binding/progress/archive/guidance and cross-command Hybrid tests, diff checks, acceptance audit, and final clean status also passed. The deliberate absence of distributed locking across machines remains a documented product limit.
- Windows Actions then exposed two platform-only delivery defects. Commit `32e278d2` (`fix(sdd): commit Windows binding rename by handle`) now opens the rooted directory through `os.Root`, supplies that handle as `RootDirectory`, and commits the simple `state.yaml` target through `NtSetInformationFile` with NT information class 65. Independent verification, amd64/386 cross-compilation, and native review lineage `review-e1e247b50e7da0f3` passed and were acknowledged.
- Windows test paths are canonicalized before exercising production's strict no-alias contract in `fae5d3d2`, `0dd07cc5`, and `310986ac`; native review lineages `review-409c270aac5d0c17`, `review-2e22b41430902f63`, and `review-6e029257c6897690` passed and were acknowledged. Commit `18eb9879` additionally replaces the timing-dependent 100 ms writer cancellation with a post-POST broadcast cancellation plus bounded watchdog; twenty repeated runs, race, independent verification, and native review lineage `review-e11b69864ec8d49f` passed.
- Delivery uses Feature Branch Chain tracker PR #734 with focused child PRs #735–#745. Each child targets its immediate parent, carries tests/docs with its cohesive work unit, names #722/#724 exclusions, and records a size exception only where an authority boundary could not be split below 400 changed lines. The exact pre-rebase reviewed history is preserved at `archive/issue-723-reviewed`. Real Windows Actions passed the rooted read-only replacement and the cumulative progress/archive/restart/guidance branches; PRs #737 and #742–#745 each finished with all 14 checks green.

## Next step

Review and integrate child PRs #735–#745 in order, then integrate draft tracker PR #734 into `master` and confirm issue #723 closes with all CI checks green.
