...
Supersession currently has a local consent gate, not a public successor command. After all prechecks, a predecessor with credited tasks requires typing the exact successor name; zero credited tasks skip the prompt. The predecessor seal retains its original task manifest. A fresh successor begins at generation 1/revision 1 with no transferred credit. Supersession does not bypass archive requirements.
