...
The Compose file builds the dashboard from `hive-dashboard/`, copies `dist` into the API container at `/app/dashboard`, and sets `DASHBOARD_ASSETS_DIR=/app/dashboard`.
...
| `JWT_SECRET` | Yes | Signs Hive API JWT sessions. Must be at least 32 bytes. |
