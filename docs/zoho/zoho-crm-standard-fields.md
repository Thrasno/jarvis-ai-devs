# Zoho CRM standard field baseline

This catalog recognizes standard Zoho CRM field display names, API names, and data types for the modules in [the standard module baseline](zoho-crm-standard-modules.md). It is not a write-safety, permission, layout, relationship, quota, or runtime-policy catalog.

The source snapshot was generated from authenticated Zoho CRM V8 Fields Metadata (`GET /crm/v8/settings/fields?module={module_api_name}`) on 2026-08-29, then filtered against the official standard CRM evidence registered in [the evidence dossier](zoho-crm-evidence-dossier.md). Only rows marked `custom_field: false` in that snapshot are retained. Runtime metadata remains authoritative for the target organization.

Field identity is `(module_api_name, field_api_name)`. Field API name is authoritative. Display labels are recognition hints and may be localized or renamed in the observed tenant. Data type is retained as snapshot recognition metadata, not as an operation-safety contract.
