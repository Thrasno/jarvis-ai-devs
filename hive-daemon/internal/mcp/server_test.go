...
func (m *mockStore) SavePromptWithSession(ctx context.Context, in models.PromptWrite) (*models.Prompt, error) {
	if m.savePromptWithSessionFn != nil {
		return m.savePromptWithSessionFn(ctx, in)
	}
	return &models.Prompt{ID: 1, Project: in.Session.Project, SessionID: in.Session.ID, Content: in.Content, CreatedAt: time.Now()}, nil
}
...
func (m *mockStore) EnsureSession(ctx context.Context, in models.SessionInput) (*models.Session, error) {
...
func (m *mockSyncer) Drain(ctx context.Context, project string, policy hivesync.TriggerPolicy) (*hivesync.Result, hivesync.DrainOutcome, error) {
	...
	return result, hivesync.DrainOutcome{BatchesDone: 1, State: hivesync.DrainFullySynced}, err
}
